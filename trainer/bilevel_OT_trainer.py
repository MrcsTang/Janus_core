...
def merged_lora_state_dict(adapter):
    """
    Merges LoRA weights and returns the state dict of the merged model.
    Handles quantized models correctly.
    """
    if hasattr(adapter, 'merge_and_unload'):
        # For PEFT models, this is the recommended way to get the merged state dict
        merged_model = adapter.merge_and_unload()
        state_dict = merged_model.state_dict()
        # We need to reload the adapter to continue training
        adapter.load_adapter()
        return state_dict
    else:
        # Fallback for non-PEFT models or older versions
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        if hasattr(self.ctx, 'init_adap'):
            ...
            for init_adap_param, cur_adap_param in zip(
                ...
                if init_adap_param.dtype == torch.bool or cur_adap_param.dtype == torch.bool:
                    continue
