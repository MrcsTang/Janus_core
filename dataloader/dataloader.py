...
class LLMDataCollator(object):
    ...
    def __call__(self, instances):
        ...
        labels_padded = self.tokenizer.pad(
            {"input_ids": labels_list}, # 同样使用 input_ids 作为键
            padding=True,
            return_tensors="pt"
        )

        # 将 labels 中由填充产生的部分替换为 IGNORE_INDEX
        # tokenizer.pad 使用 pad_token_id 填充，我们需要将其替换为-100
        labels = labels_padded["input_ids"]
        labels[labels == self.tokenizer.pad_token_id] = DefaultToken.IGNORE_INDEX.value
